...
class PostgresDatabaseConnection(DatabaseConnection):
    ...
    def run_query_collect_statistics(self,
        ...
        if mode == ExecutionMode.JSON_OUTPUT:
            ...
            verbose_query = "(verbose true, analyze false, format json)"

        elif mode == ExecutionMode.RAW_OUTPUT:
            analyze_query =  "(analyze true)"
            verbose_query = "(verbose true, analyze false)"
        else:
            raise Exception("Execution mode not supported")

        try:
            verbose_plan = self.get_result(f"{prefix} explain {verbose_query} {sql}",
                                           include_hint_notices=include_hint_notices, mode=mode, hintsets=hintsets)

            if verbose_plan is None:
                return None
            analyze_plans = []
            if not explain_only:
                for i in range(repetitions):
                    statement = f"{prefix} explain {analyze_query} {sql}"
                    curr_analyze_plan = self.get_result(statement, include_hint_notices=include_hint_notices, mode=mode, hintsets=hintsets)
    ...
    def get_result(self, sql: str,
                   ...
                   include_hint_notices: bool = False,
                   mode: ExecutionMode = ExecutionMode.RAW_OUTPUT,
                   hintsets: str = ""):
        connection, cursor = self.get_cursor(db_created=db_created)
        try:
            # 执行 hint 语句（如有）
            if hintsets:
                if isinstance(hintsets, list):
                    for hint in hintsets:
                        if hint.strip():
                            cursor.execute(hint.strip())
                elif isinstance(hintsets, str) and hintsets.strip():
                    cursor.execute(hintsets.strip())
            ...
            if include_hint_notices:
                if not connection.notices:
                    print(f"[Hint Warning] No hint logs found during execution of:\n{sql}")
                    return [], records
            ...
            if mode == ExecutionMode.JSON_OUTPUT:
                ...
                psycopg2.errors.IndeterminateDatatype,
                psycopg2.errors.QueryCanceled) as e:
            print(f"[PostgreSQL Error] Skipped query due to error: {type(e).__name__} -- {str(e).strip()}\nSQL:\n{sql}")
            return None

        except psycopg2.Error as e:
            # 捕获所有其他数据库错误
            print(f"[PostgreSQL Error] Unhandled psycopg2 error: {type(e).__name__} -- {str(e).strip()}\nSQL:\n{sql}")
            return None
    ...
    #                include_hint_notices: bool = False, mode: ExecutionMode = ExecutionMode.RAW_OUTPUT, hintsets: str=""):
    #     connection, cursor = self.get_cursor(db_created=db_created)
    #     if hintsets:
    #         if isinstance(hintsets, list):
    #             for hint in hintsets:
    #                 if hint.strip():  # 防止空字符串
    #                     cursor.execute(hint.strip())
    #         elif isinstance(hintsets, str) and hintsets.strip():
    #             cursor.execute(hintsets.strip())
    #     cursor.execute(sql)
